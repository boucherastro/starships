...
def main(config_filepath, visit_name, model_path, scaling_factor, RV_inj, out_dir = None, path_fig = '', debug = False):
    ...
    p, obs = red.load_planet(config_dict, visit_name)

    # set save directory
    if out_dir == None:
        scratch_dir = Path(os.environ["SCRATCH"])
        scratch_dir /= Path(f'{config_dict["instrument"]}/Reductions/injection_recovery/{"".join(p.name.split())}')
    ...
    # loading in the model and scale it
    wave_mod, mod_spec = np.load(model_path)['wave_mod'], np.load(model_path)['mod_spec']
    mod_spec_scaled = (mod_spec - (p.R_pl / p.R_star)**2)* scaling_factor 
    ...
    v_shift = v_orb[0].value + v_sys[0].value + RV_inj + berv[0]
...
for i, exp in enumerate(exp_list):
    ...
    v_shift = v_orb[i].value + v_sys[0].value + RV_inj + berv[i]
    wave_mod_shifted = wave_mod * (1 + v_shift / 299792.458)
