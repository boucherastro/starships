...
from starships.transpec import resolve_reference_spec_exposures


class TestResolveReferenceSpecExposures:
    """`resolve_reference_spec_exposures` (Chantier B, B1) makes the reference
    spectrum's exposure selection a real, explicit choice. Before this
    ...
    def test_all_string_uses_every_exposure(self):
        result = resolve_reference_spec_exposures('all', iOut=np.array([1, 2]), n_exposures=5)
        np.testing.assert_array_equal(result, np.arange(5))

    def test_none_uses_true_out_of_transit_exposures(self):
        iOut = np.array([0, 1, 4])
        result = resolve_reference_spec_exposures(None, iOut=iOut, n_exposures=5)
        np.testing.assert_array_equal(result, iOut)

    def test_explicit_array_passed_through(self):
        explicit = np.array([2, 3])
        result = resolve_reference_spec_exposures(explicit, iOut=np.array([0, 1]), n_exposures=5)
    ...
    def test_oversized_iout_temp_falls_back_to_all(self):
        ...
        result = resolve_reference_spec_exposures(oversized, iOut=np.array([0]), n_exposures=5)
        np.testing.assert_array_equal(result, np.arange(5))
