...
def get_ker(theta_regions, tr_i=0, phase=None, planet=None, instrum=None, model_resolution=None):
    return {
        "phase": phase,
        "planet": planet,
        "instrum": instrum,
...
class TestLoadCustomGetKer:
    ...
    def test_loads_module_level_get_ker(self, tmp_path):
        ...
        result = get_ker([{}, {}], tr_i=1, phase=0.5, planet="dummy_planet",
    ...
    def test_loaded_function_does_not_need_retrieval_globals(self, tmp_path):
        ...
        the get_ker file used to reference `data_trs`/`planet`/`instrum`/`lbl_res`
        ...
        get_ker([{}], tr_i=0, phase=0.1, planet=None, instrum=None, model_resolution=None)
...
class TestPrepareModelMultiRegGetKerWiring:
    ...
    `data_trs`/`planet`) and passes it -- along with `planet`, the visit's
    ...
    def test_computes_phase_and_forwards_context_to_get_ker(self, monkeypatch):
        ...
        def fake_get_ker(theta_regions, tr_i=0, phase=None, planet=None, instrum=None,
            ...
            calls.append(dict(tr_i=tr_i, phase=phase, planet=planet, instrum=instrum,
                              model_resolution=model_resolution))
        ...
        monkeypatch.setattr(retrieval, 'prepare_model_high_or_low', fake_prepare_model_high_or_low,
                            raising=False)
        monkeypatch.setattr(retrieval, 'data_trs', data_trs_fake, raising=False)
        ...
        retrieval.prepare_model_multi_reg(theta_regions, 'high', tr_i=0)
